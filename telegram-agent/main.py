...
def search_web(chat_id, query):
    ...
    prompt = (
        ...
        "جاوب على سؤال المستخدم مباشرة، بأسلوب محادثة طبيعي وقصير كأنك ترد على صديق، "
        "بالاعتماد على هذي النتائج فقط (لا على معلوماتك السابقة عن التاريخ أو الأحداث). "
        "لا تذكر كلمة 'نتائج البحث' ولا تسرد مصادر أو روابط، فقط جاوب على السؤال مباشرة."
    )
...
def webhook():
    ...
    if not text and photo:
        ...
        prompt = (
            message.get("caption")
            or "صف هذي الصورة واشرح لي وش فيها. لو فيها جدول أو معلومات منظمة، استخرجها بشكل واضح ومنظم."
        )
        reply = ask_gemini_vision(chat_id, prompt, image_bytes, "image/jpeg")
        save_note(chat_id, f"[من صورة أرسلها المستخدم] {reply}")
        send_message(chat_id, reply)
        return jsonify(ok=True)
